...
/**
 * Creates or handles an OtherInformation contained in an External
 * @version %W% %G%
 * @author Ralph LeVan
 */
...
public class ExternalOtherInformation extends External {
    ...
    public ExternalOtherInformation() {
        super(OtherInformationOID, (DataDir)null);
        otherInfo=new OtherInformation();
        add(otherInfo);
    }


    public ExternalOtherInformation(DataDir dir) {
        super(dir);
        otherInfo=new OtherInformation(asn1Data());
    }


    public DataDir add(External e) {
        return otherInfo.add(e);
    }


    public Enumeration elements() {
        return new OtherInformationEnumeration(otherInfo);
    }


    public boolean OK() {
        if(fldid()==ASN1.EXTERNAL)
            return true;
        return false;
    }
}
